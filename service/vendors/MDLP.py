...
import pandas as pd
import numpy as np
from service.vendors.Entropy import entropy, cut_point_information_gain
...
class MDLP_Discretizer(object):
    def __init__(self, X, y, features=None):
        ...
        self._cuts = {f: [] for f in self._data.columns}
    ...
    def single_feature_accepted_cutpoints(self, feature, cuts, partition_index=pd.DataFrame().index):
        '''
        ...
        # apply decision
        if not decision:
            return feature, cuts  # if partition wasn't accepted, there's nothing else to do
        if decision:
            # try:
            ...
            if left_partition.empty or right_partition.empty:
                return feature, cuts  # extreme point selected, don't partition
            cuts.append(cut_candidate)  # accept partition
            self.single_feature_accepted_cutpoints(feature=feature, cuts=cuts, partition_index=left_partition.index)
            self.single_feature_accepted_cutpoints(feature=feature, cuts=cuts, partition_index=right_partition.index)
    ...
    def all_features_accepted_cutpoints(self):
        ...
        results = Parallel(n_jobs=cpu_count, verbose=True) \
            (delayed(self.single_feature_accepted_cutpoints)(attr, self._cuts[attr]) \
             for attr in self._features)
    ...
    def apply_cutpoints(self):
        ...
        # if not out_data_path or not out_bins_path:
        self.dataset = self._data[self._features]
        # # save data as csv
        # if out_data_path:
        ...
        #                 attr, ', '.join([bin_label for bin_label in bin_label_collection[attr]]))
