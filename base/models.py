...
class BaseModel(Model):
    ...
    @gen.coroutine
    def update(self, db=None, query=None, collection=None, update=None, ser=None, upsert=False, multi=False):
        """
    ...
    @classmethod
    def get_cursor(cls, db, query, collection=None, fields={}, sort_fields={}):
        c = cls.check_collection(collection)
        query = cls.process_query(query)
        return db[c].find(query, fields) \
            if fields else db[c].find(query)
    ...
    def reconnect_amount():
        return range(mongo_settings['reconnect_tries'] + 1)

    @classmethod
    @gen.coroutine
    def check_reconnect_tries_and_wait(cls, reconnect_number, func_name):
        if reconnect_number >= mongo_settings['reconnect_tries']:
